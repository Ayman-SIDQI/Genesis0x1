...
/*   Updated: 2023/01/27 12:12:50 by asidqi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	putmap(t_map *all)
{
	...
	while (all->splitted[++all->z])
	{
		...
		while (all->splitted[all->z][all->o] != '\0')
		{
			...
			else if (all->splitted[all->z][all->o] == 'Y')
				putblk(all, all->o, all->z, all->enemy);
...
void	susume(t_map *all, int y, int x, void *img)
{
	...
	if (all->splitted[all->enemyy][all->enemyx] \
		== all->splitted[all->playery][all->playerx])
		fd_mlx_fp_bonus(all->full, all->tmp, all->splitted, NULL);
...
int	coordinator(int button, t_map *all)
{
	...
	if ((button == UP || button == 126) && checksafe(all, 1, 0))
	{
		susume(all, 1, 0, all->sup);
		incrementor(all, ++n);
	}
	else if ((button == DOWN || button == 125) && checksafe(all, -1, 0))
	{
		susume(all, -1, 0, all->standing);
		incrementor(all, ++n);
	}
	else if ((button == LEFT || button == 123) && checksafe(all, 0, 1))
	{
		susume(all, 0, 1, all->lisr);
		incrementor(all, ++n);
	}
	else if ((button == RIGHT || button == 124) && checksafe(all, 0, -1))
	{
...
void	bitwix(t_map *all)
{
	...
	putmap(all);
	size_and_width(all);
	mlx_loop_hook(all->mlx_ptr, animation_jutsu, all);
